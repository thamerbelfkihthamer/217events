		...
		<div id="first" style="top:{{ top1 }}">
			<div id="by" style="opacity:{{bydisplay}}">
				...
				<a href="https://www.facebook.com/thamer.belfkih" target="_blank">
					<i class="fa fa-code"></i> <span>Belfkih Thamer</span>
				</a>
			</div>
			<header>
...
Le destin de la Tunisie est entre nos mains. Nous ne pouvons pas rater cette occasion !
		...
		<div class="holder" id="holder" onscroll="showShadow()" style="opacity:{{fopacity}}">
			<div ng-repeat="formateur in formateurs | random | filter:search" class="formateur">
				...
				<div ng-if="formateur.full1<1 || formateur.full2<1" class="image" style="background-image:url({{ formateur.image }})" ng-click="showFormf(formateur,formateurs.indexOf(formateur) + 1)"> </div> 
				<div ng-if="formateur.full1>=1 && formateur.full2>=1" class="image" style="background-image:url({{ formateur.image }});opacity:0.2" ng-click="showFormf(formateur,formateurs.indexOf(formateur) + 1)"> </div> <br>
				...
				<div class="text">
					...
					<br />
					<span class="formation">{{ formateur.nom_formation | utf8 }}</span>
				</div>
			</div>
		</div>
...
Commençons à prendre des initiatives, à travailler pour construire un meilleur avenir à la Tunisie !
				...
				<button class="pure-button pure-button-primary" ng-click="add()" disbaled>valider</button>
			</div>
		</div>
		...
		<button class="button-warning pure-button close" ng-click="closeform()">x</button>
			<div class="holder2">
				...
				<span class="label">Session: (1:matin, 2:apres midi)</span> <select id="ss" ng-model="session">
					<option id="opt1" value="1">1</option><option id="opt2" value="2">2</option></select>
				<button class="pure-button pure-button-primary" ng-click="addVisiteur()"  >valider</button>
				</div>
